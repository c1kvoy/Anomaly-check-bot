...
async def get_stock_data(symbol, api_key):
...
async def process_all_stocks():
    for index, row in df.iterrows():
        symbol = row['Ticker']
        api_key = row['key']
        await get_stock_data(symbol, api_key)
...
async def calculate_percent(name, values):
    if len(values) < 2:
        print(f"Недостаточно данных для символа {name}.")
        return False, 0, name
...
async def main():
    ...
    while True:
        analysis_data = await process_all_stocks()
